...
module.exports = {
  publicPath: IS_PROD ? process.env.VUE_APP_PUBLIC_PATH : '/', // 默认'/'，部署应用包时的基本URL
  ...
  devServer: {
    ...
    proxy: {
      // 例如将'http://localhost:8089/api/xxx'代理到'http://localhost:8090/api/xxx'
      '/api': {
        target: 'http://127.0.0.1:8090', // 目标代理接口地址
        secure: false, // 忽略https安全提示(如果是https接口，需要配置这个参数)
        changeOrigin: true, // 本地会虚拟一个服务器接收请求并代发该请求
        ws: true, // 启用websockets
        pathRewrite: { // 重写地址，将前缀 '/api' 转为 '/',相当于此时代理到'http://127.0.0.1:8090/xxx'
          '^/api': '/ke'
        }
      }
    }
